...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/lp_contrastive.py \
    ...
    --alg=fitted_sarsa_c_learning_reverse_kl \
    --seed="$seed" \
    --lp_launch_type=local_mt \
    --root_dir=~/offline_c_learning/contrastive_rl_logs/offline/"${EXP_LABEL}"/offline_ant_medium_play/$seed \
    --trained_agent_dir=~/offline_c_learning/contrastive_rl_logs/offline/jul_22_contrastive_nce_reverse_kl_coef_sum_to_1/offline_ant_medium_play/0/checkpoints/learner \
