...
class FetchSlideImageEnv(slide.FetchSlideEnv):
    ...
    def reset(self):
        ...
        # Randomize object position
        # for _ in range(8):
        #     super(FetchSlideImageEnv, self).step(np.array([-1.0, 0.0, 0.0, 0.0]))
        object_qpos = self.sim.data.get_joint_qpos('object0:joint')
        ...
        # else:
        #     for _ in range(5):
        #         super(FetchSlideImageEnv, self).step(self.action_space.sample())
        self._move_hand_to_obj()
        ...
        # obs_img = Image.fromarray(img.reshape([400, 400, 3]))
        # goal_img = Image.fromarray(self._goal_img.reshape([400, 400, 3]))
        # obs_img.save("/projects/rsalakhugroup/chongyiz/td_cpc_logs/debug/fetch_slide_obs.png")
        # goal_img.save("/projects/rsalakhugroup/chongyiz/td_cpc_logs/debug/fetch_slide_goal.png")
        #
        # exit()
...
class FetchPickAndPlaceImageEnv(pick_and_place.FetchPickAndPlaceEnv):
    ...
    def _pick_and_place(self):
        ...
        for _ in range(50):
            hand = s['observation'][:3]
            # obj = s['achieved_goal'] + np.array([0.0, 0.0, 0.0])
            # obj = s['achieved_goal'] + np.array([0.01, 0.0, -0.01])
    ...
    def reset(self):
        ...
        while not is_success:
            ...
            # goal in the air
            # # if self.goal[2] <= s['achieved_goal'][2] + 0.05:
            # if self.goal[2] <= s['achieved_goal'][2] + 0.4:
            #     print("desk goal")
            #     continue
        ...
        # obs_img = Image.fromarray(img.reshape([400, 400, 3]))
        # goal_img = Image.fromarray(self._goal_img.reshape([400, 400, 3]))
        # obs_img.save("/projects/rsalakhugroup/chongyiz/td_cpc_logs/debug/fetch_pick_and_place_obs.png")
        # goal_img.save("/projects/rsalakhugroup/chongyiz/td_cpc_logs/debug/fetch_pick_and_place_goal.png")
        #
        # exit()
    ...
    def observation(self, observation):
        self.sim.data.site_xpos[0] = 1_000_000
        img = self.render(mode='rgb_array', height=64, width=64)
        # img = self.render(mode='rgb_array', height=400, width=400)
        return img.flatten()
