...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_umaze_v2/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --gin_bindings="train_eval.log_subset=(0, 2)" \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_umaze_v2/$seed \
  > ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_umaze_v2/$seed/stream.log 2>&1 & \
  sleep 2
done
