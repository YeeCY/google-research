...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_logs_debug/"${EXP_LABEL}"/metaworld_reach/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs_debug/"${EXP_LABEL}"/metaworld_reach/$seed \
  > ~/offline_c_learning/c_learning_logs_debug/"${EXP_LABEL}"/metaworld_reach/$seed/stream.log 2>&1 & \
  sleep 2
done
