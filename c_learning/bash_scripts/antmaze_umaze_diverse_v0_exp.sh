...
for seed in "${seeds[@]}"; do
  ...
  python $PROJECT_DIR/train_eval.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_umaze_diverse_v0/$seed \
  > ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_umaze_diverse_v0/$seed/stream.log 2>&1 &
done
