...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_large_diverse_v2/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_large_diverse_v2/$seed \
  > ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/antmaze_large_diverse_v2/$seed/stream.log 2>&1 &
done
