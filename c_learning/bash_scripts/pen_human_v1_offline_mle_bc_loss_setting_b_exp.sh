...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/pen_human_v1/$seed
  nohup \
  python $PROJECT_DIR/train_eval_offline_d4rl.py \
    --gin_bindings="train_eval_offline.env_name='pen-human-v1'" \
    --gin_bindings="train_eval_offline.random_seed=${seed}" \
    --gin_bindings="train_eval_offline.num_iterations=1000000" \
    --gin_bindings="train_eval_offline.max_future_steps=65" \
    --gin_bindings="obs_to_goal.start_index=(39,)" \
    --gin_bindings="obs_to_goal.end_index=(45,)" \
    ...
    --root_dir ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/pen_human_v1/$seed \
  > ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/pen_human_v1/$seed/stream.log 2>&1 & \
  sleep 5
done
