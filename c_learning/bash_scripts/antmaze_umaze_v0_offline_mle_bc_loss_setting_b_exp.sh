...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_umaze_v0/$seed
  nohup \
  python $PROJECT_DIR/train_eval_offline_d4rl.py \
    --gin_bindings="train_eval_offline.env_name='antmaze-umaze-v0'" \
    ...
    --root_dir ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_umaze_v0/$seed \
  > ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_umaze_v0/$seed/stream.log 2>&1 & \
  sleep 5
done
