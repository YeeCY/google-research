...
for seed in "${seeds[@]}"; do
  mkdir -p ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/maze2d_open_v0/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --gin_bindings="train_eval.max_future_steps=50" \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/maze2d_open_v0/$seed \
  > ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/maze2d_open_v0/$seed/stream.log 2>&1 &
done
