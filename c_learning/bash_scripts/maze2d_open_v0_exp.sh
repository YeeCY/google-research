...
for seed in "${seeds[@]}"; do
  mkdir -p ~/offline_c_learning/c_learning_logs/"${DATE}"/maze2d_open_v0/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${DATE}"/maze2d_open_v0/$seed \
  > ~/offline_c_learning/c_learning_logs/"${DATE}"/maze2d_open_v0/$seed/stream.log 2>&1 &
done
