...
for seed in "${seeds[@]}"; do
  mkdir -p ~/offline_c_learning/c_learning_offline_logs/"${DATE}"/sawyer_reach_600k/$seed
  nohup \
  python $PROJECT_DIR/train_eval_offline.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_offline_logs/"${DATE}"/sawyer_reach_600k/$seed \
    --dataset_dir ~/offline_c_learning/c_learning_logs/"${DATE}"/sawyer_reach_600k/$seed/train \
  > ~/offline_c_learning/c_learning_offline_logs/"${DATE}"/sawyer_reach_600k/$seed/stream.log 2>&1 &
done
