...
for seed in "${seeds[@]}"; do
  mkdir -p ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/sawyer_reach_600k/$seed
  nohup \
  python $PROJECT_DIR/train_eval.py \
    ...
    --root_dir ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/sawyer_reach_600k/$seed \
  > ~/offline_c_learning/c_learning_logs/"${EXP_LABEL}"/sawyer_reach_600k/$seed/stream.log 2>&1 &
done
