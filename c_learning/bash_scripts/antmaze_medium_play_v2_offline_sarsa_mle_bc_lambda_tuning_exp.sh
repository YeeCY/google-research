...
declare -a all_bc_lambdas=(0.05 0.1 0.2 0.3 0.5 1.0)
declare -a seeds=(0)

for bc_lambda in "${all_bc_lambdas[@]}"; do
  for seed in "${seeds[@]}"; do
    rm $CONDA_PREFIX/lib/python*/site-packages/mujoco_py/generated/mujocopy-buildlock
    mkdir -p ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_medium_play_v2_bc_lambda="${bc_lambda}"/$seed
    nohup \
    ...
    > ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_medium_play_v2_bc_lambda="${bc_lambda}"/$seed/stream.log 2>&1 & \
    sleep 5
  done
done
