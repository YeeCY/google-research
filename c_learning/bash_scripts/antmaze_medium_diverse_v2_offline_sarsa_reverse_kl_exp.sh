...
for seed in "${seeds[@]}"; do
  ...
  mkdir -p ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_medium_diverse_v2/$seed
  nohup \
  python $PROJECT_DIR/train_eval_offline_d4rl.py \
    --gin_bindings="train_eval_offline.agent='c_learning_agent'" \
    --gin_bindings="train_eval_offline.env_name='antmaze-medium-diverse-v2'" \
    ...
    --gin_bindings="c_learning_agent.actor_loss.reverse_kl_loss=True" \
    --gin_bindings="c_learning_agent.actor_loss.reverse_kl_lambda=0.25" \
    --root_dir ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_medium_diverse_v2/$seed \
  > ~/offline_c_learning/c_learning_offline_logs/"${EXP_LABEL}"/antmaze_medium_diverse_v2/$seed/stream.log 2>&1 & \
  sleep 2
done
